...
public class ScheduleHandlerHelper {
    ...
    private static ImmutableMap<ImmutablePair<Action, Type>, ScheduleFunction> choices =
            new ImmutableMap.Builder<ImmutablePair<Action, Type>, ScheduleFunction>()
                    ...
                    .put(p(Action.WEEK, Type.GROUP), (ScheduleFunction) (id, service) -> {
                        ImmutablePair<Group, Map<LocalDate, List<Lesson>>> response = service.week(id, getNow(),
                                Group.class);
                        if (response.left != null) {
                    ...
                    .put(p(Action.WEEK, Type.TEACHER), (ScheduleFunction) (id, service) -> {
                        ImmutablePair<Teacher, Map<LocalDate, List<Lesson>>> response = service.week(id, getNow(),
                                Teacher.class);
                        if (response.left != null) {
                            return ImmutablePair.of(response.left.getName(), response.right);
                        }
                    ...
                    .put(p(Action.WEEK, Type.ROOM), (ScheduleFunction) (id, service) -> {
                        ImmutablePair<Room, Map<LocalDate, List<Lesson>>> response = service.week(id, getNow(),
                                Room.class);
                        if (response.left != null) {
                            return ImmutablePair.of(response.left.getName(), response.right);
                        }
                    ...
                    .put(p(Action.NEXT_WEEK, Type.GROUP), (ScheduleFunction) (id, service) -> {
                        ImmutablePair<Group, Map<LocalDate, List<Lesson>>> response = service.week(id,
                                getNow().plusDays(7), Group.class);
                        if (response.left != null) {
                            return ImmutablePair.of(response.left.getName(), response.right);
                        }
                        return ImmutablePair.of(null, null);
                    ...
                    .put(p(Action.NEXT_WEEK, Type.TEACHER), (ScheduleFunction) (id, service) -> {
                        ImmutablePair<Teacher, Map<LocalDate, List<Lesson>>> response = service.week(id,
                                getNow().plusDays(7), Teacher.class);
                        if (response.left != null) {
                            return ImmutablePair.of(response.left.getName(), response.right);
                        }
                        return ImmutablePair.of(null, null);
                    ...
                    .put(p(Action.NEXT_WEEK, Type.ROOM), (ScheduleFunction) (id, service) -> {
                        ImmutablePair<Room, Map<LocalDate, List<Lesson>>> response = service.week(id,
                                getNow().plusDays(7), Room.class);
                        if (response.left != null) {
                            return ImmutablePair.of(response.left.getName(), response.right);
                        }
                        return ImmutablePair.of(null, null);
...
//        return LocalDate.of(2019, 4, 22);
    ...
    public interface ScheduleFunction
            extends BiFunction<Long, LessonService, ImmutablePair<String, Map<LocalDate, List<Lesson>>>> {
    }
